    ...
    <>
      ...
      <div className="text-white rounded-2xl px-5 py-5 mb-6 flex flex-wrap items-end justify-between gap-3" style={{ background: "var(--color-cyan)" }}>
        <div>
          ...
          <p className="text-white/80 mt-1 text-sm">{slep?.nombre} · Programa Aprender en Familia</p>
        </div>
      ...
      <div className="card mb-8">
        <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
          <div>
            ...
            <h3 className="text-lg text-gray-dark">Tu establecimiento vs promedio {slep?.nombre}</h3>
          </div>
        </div>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={AMBITOS.map(a => ({
              ...
              <Bar dataKey="Promedio SLEP" fill="rgb(255,220,0)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex items-start justify-between gap-3 mb-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-gray-ui mt-1">{filasIndicadores.length} indicadores · {filasIndicadores.filter(f => f.ind.tipo === 'operativo').length} operativos · {filasIndicadores.filter(f => f.ind.tipo === 'táctico').length} tácticos</p>
          </div>
        </div>
          ...
          return (
            <div key={a.id} className="mb-6 last:mb-0">
              <div className="flex items-center gap-2 mb-3 pb-2 border-b border-border">
                ...
                <h4 className="text-base text-gray-dark">{a.nombre}</h4>
              </div>
              <div className="space-y-2">
                {filas.map(({ ind, valor, logro }) => (
                  <div key={ind.id} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 py-2.5 px-3 rounded-xl hover:bg-bg transition">
                    <div className="flex items-start sm:items-center gap-3 flex-1 min-w-0">
                      <div className="w-12 text-xs text-gray-ui font-mono shrink-0 pt-0.5 sm:pt-0">{ind.id}</div>
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 pl-[60px] sm:pl-0">
                      ...
                      <div className="text-right shrink-0 w-24 sm:w-32">
                        ...
                        <p className="text-xs text-gray-ui">meta: {ind.meta}</p>
                      </div>
